...
// Larguras reais do slot, medidas: 640px na coluna do desktop, e a largura da
// tela menos o padding lateral no empilhado. Sem isso o celular baixa a prancha
// de desktop para um slot de 300px.
const TAMANHOS = '(min-width: 1024px) 640px, (min-width: 640px) calc(100vw - 5rem), calc(100vw - 3rem)'

function Prancha({ espaco, className = '', loading = 'lazy', priority }) {
  return (
    <picture className="contents">
      <source srcSet={espaco.avif} sizes={TAMANHOS} type="image/avif" />
      <source srcSet={espaco.webp} sizes={TAMANHOS} type="image/webp" />
      <img
        src={espaco.src}
        srcSet={espaco.webp}
        sizes={TAMANHOS}
...
/** Cabeçalho comum às duas variantes: numeral, fio e etiqueta. */
function Cabecalho({ espaco }) {
  return (
...
function Parada({ espaco, indice, onAtivar, ultimo }) {
  ...
  return (
    <div
      ref={ref}
      id={espaco.id}
      className={`scroll-mt-28 ${ultimo ? 'min-h-[70svh]' : 'min-h-[85svh]'} flex flex-col justify-center`}
    >
      <Cabecalho espaco={espaco} />
...
/** Desktop: a prancha fica parada e o texto avança por cima do percurso. */
function PercursoFixo() {
  const [ativo, setAtivo] = useState(0)
  const semMovimento = useReducedMotion()
  const atual = espacos[ativo]

  return (
    <div className="max-w-7xl mx-auto px-10 xl:px-16 py-24">
...
/** Mobile e tablet: sem prancha fixa, cada ambiente é um bloco na sequência. */
function PercursoEmpilhado() {
  return (
...
// As duas variantes carregam os mesmos `id` — são os destinos do índice na
// planta. Por isso elas não podem coexistir no DOM sob `hidden lg:block`:
// o navegador resolve `#tenis` para a primeira ocorrência mesmo quando ela
// está dentro de um `display:none`, e no celular todo link do índice levava
// ao topo da página. Aqui só uma das duas é montada.
export default function PercursoEspacos() {
  ...
  return (
    <section aria-label="Percurso pelos ambientes do clube" className="bg-paper">
      {noDesktop ? <PercursoFixo /> : <PercursoEmpilhado />}
    </section>
  )
}
